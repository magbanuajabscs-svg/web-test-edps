...
// Kukunin ang lahat ng projects/events mula sa database
$allEvents = $projectModel->getAll();
?>
<!DOCTYPE html>
...
<head>
    ...
    <title>Project List | EDPS Studio</title>
...
<body class="bg-[#12100E] font-body text-[#D1CDC7] antialiased flex min-h-screen">
        ...
        <aside class="hidden md:flex flex-col h-screen w-72 border-r border-[#2C1A12] bg-[#1A1614] py-8 px-6 sticky top-0 font-['Noto_Serif'] antialiased overflow-y-auto">
            ...
            <nav class="flex-1 space-y-2">
                ...
                <a class="flex items-center gap-4 text-[#D1CDC7] pl-5 py-3 hover:bg-[#2C1A12] transition-colors duration-200 group" id="projects-nav-link" href="stafflist.php">
...
<main class="flex-1 flex flex-col min-w-0">
    ...
    <section class="p-8 md:p-12 lg:p-20 max-w-7xl mx-auto w-full">
        <div class="mb-12">
            <h3 class="text-4xl md:text-5xl lg:text-6xl font-headline font-bold text-[#E4E0DC] tracking-tight mb-4 leading-tight">Project Monitoring</h3>
            <p class="text-[#D1CDC7] max-w-xl text-lg font-body">Overview of all studio project and their current production status.</p>
            
            <div class="flex border-b border-outline-variant/20 mt-10">
                <a href="stafflist.php" class="px-6 py-3 text-[#D1CDC7] hover:text-[#C5A059] transition-colors font-bold text-sm uppercase tracking-widest">
                    Staff 
                </a>
                <a href="eventlist.php" class="px-6 py-3 border-b-2 border-[#C5A059] text-[#C5A059] font-bold text-sm uppercase tracking-widest">
                    Project
                </a>
            </div>
        </div>
        ...
        <div class="flex flex-col md:flex-row gap-4 mb-10 items-center">
            <div class="relative w-full md:flex-1">
                ...
                <input class="w-full bg-[#2a1a12] border-none py-5 pl-12 pr-6 rounded-lg text-lg font-body focus:ring-1 focus:ring-[#C5A059]/30 placeholder:text-[#D1CDC7] text-[#F5F5F0]" placeholder="Search projects by name or client..." type="text"/>
            </div>
            ...
            <?php if (empty($allEvents)): ?>
                <div class="mt-16 text-center py-20">
                    <div class="flex items-center justify-center gap-3 mb-6">
                        <span class="material-symbols-outlined text-5xl text-[#D1CDC7]/30">event_busy</span>
                    </div>
                    <h3 class="text-3xl font-headline text-[#E4E0DC] mb-3">No Projects Recorded</h3>
                    <p class="text-[#D1CDC7] mb-8 max-w-md mx-auto">Start by creating a new project to track studio events and assignments.</p>
                </div>
            <?php else: ?>
                    ...
                    <?php foreach ($allEvents as $event): ?>
                        <div class="group bg-[#2a1a12] hover:bg-[#3a2b20] transition-all duration-300 p-6 md:p-8 flex flex-col md:flex-row md:items-center justify-between gap-6 border-b border-[#3a2b20]">
                            <div class="flex items-center gap-8">
                                <div class="relative flex-none w-20 h-20 md:w-24 md:h-24 bg-[#3a2b20] rounded-lg flex items-center justify-center">
                                    <span class="material-symbols-outlined text-[#C5A059] text-4xl">photo_camera</span>
                                </div>
                                ...
                                <div>
                                    ...
                                    <h4 class="text-2xl md:text-3xl font-headline font-bold text-[#F5F5F0] leading-tight">
                            ...
                            <div class="flex items-center gap-4">
                                <a href="projectdetails.php?id=<?php echo $event['id']; ?>" class="px-8 py-3 bg-[#C5A059] text-[#1A1614] font-bold rounded-lg hover:bg-[#D4A76A] transition-colors flex items-center justify-center gap-2 whitespace-nowrap">
                                    View Project Details <span class="material-symbols-outlined text-sm">arrow_forward</span>
                                </a>
                            </div>
                        </div> 
                    <?php endforeach; ?>
...
<script>
// Fix sidebar active state for Projects navigation
// The Projects link should be active on both stafflist.php and eventlist.php
(function() {
    const currentPage = window.location.pathname;
    const projectsNavLink = document.getElementById('projects-nav-link');
    
    // Check if we're on eventlist.php and mark the link as active
    if (currentPage.includes('eventlist.php') && projectsNavLink) {
        projectsNavLink.className = 'flex items-center gap-4 text-[#F5F5F0] font-bold border-l-4 border-[#C5A059] pl-4 py-3 bg-[#2C1A12] transition-colors duration-200 group';
    }
})();
</script>
</body>
