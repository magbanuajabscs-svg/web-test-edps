#!/usr/bin/env python3
"""Update staffmanage.php with Dashboard dark theme styling"""
...
# Dark theme sidebar for staffmanage
dark_sidebar_staffmanage = '''<aside class="hidden md:flex flex-col h-screen w-72 border-r border-[#2C1A12] bg-[#1A1614] py-8 px-6 sticky top-0 font-['Noto_Serif'] antialiased overflow-y-auto">
    <div class="mb-10">
...
# Dark theme header for staffmanage
dark_header_staffmanage = '''<main class="flex-1 flex flex-col min-w-0">
    <!-- TopAppBar -->
...
def update_staffmanage():
    """Update staffmanage.php with dark theme styling"""
    filepath = "staffmanage.php"
    ...
    try:
        ...
        # Replace tailwind config and style section
        pattern = r'<script id="tailwind-config">.*?</style>'
        if re.search(pattern, content, re.DOTALL):
            content = re.sub(pattern, tailwind_config + '\n', content, flags=re.DOTALL)
        
        # Replace sidebar
        pattern = r'<aside[^>]*class="hidden md:flex[^"]*"[^>]*>.*?</aside>'
        if re.search(pattern, content, re.DOTALL):
            content = re.sub(pattern, dark_sidebar_staffmanage, content, flags=re.DOTALL)
        
        # Replace header
        pattern = r'<main[^>]*class="flex-1[^"]*"[^>]*>\s*<header[^>]*class="[^"]*"[^>]*>.*?</header>'
        if re.search(pattern, content, re.DOTALL):
            content = re.sub(pattern, dark_header_staffmanage, content, flags=re.DOTALL)
        
        with open(filepath, 'w', encoding='utf-8') as f:
            f.write(content)
        
        print(f"✓ Updated staffmanage.php with dark theme")
        return True
